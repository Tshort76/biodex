...
- **GPS from the photo picker.** The picker on the Pixel 7 Pro already logs a `locationMetadataAccessRequested` intent option behind a device flag that is off (`PICKER_LOCATION_METADATA_ENABLED=false`, 2026-09-13). When it turns on, `PickVisualMediaRequest` may gain a way to ask for it; `readExif` and the geocoder are already wired for the coordinates. Check the flag again before spending time on it — D57 has the failed alternative, and D58 the working one (the Files picker, which already keeps GPS for any photo still on the phone).
- **Identify the photo in the app (owner-approved idea, 2026-09-24).** The Identify screen (D78) names a species by sending the photo to Google Lens and pasting the answer back. The alternative the owner deferred: a button there that sends the photo to an AI vision API (Gemini or Claude) and fills in the name itself — no Lens round-trip, no copy and paste. It needs the owner's own API key typed into Settings and kept on the phone (never in the repo, the same rule Pl@ntNet's key followed until v21), costs a fraction of a cent a photo, and sends each photo to that service. Lens answered only to the family for a worn moth in testing, so compare the two on the owner's hard cases before choosing.
...
- **An on-device model that identifies the species in a photograph.** (Animals and fungi only now — plants left the dex in v21, D59, and the owner has other apps for them.) The owner's framing was "training an object detection model for species classification", which is really two jobs: *detection* — find the animal or plant in the frame, which would also give the reference-photo crop something better than a centred fit — and *classification* — decide which of the 230 it is. This does not contradict `D23`, which ruled out third-party identification providers for animals and fungi; it reopens the same want by a different route, since a first-party model answers to our own catalogue instead of a global one and can honestly say "not in dex". What makes it large is not the training loop: it is a labelled image corpus we are allowed to use (the owner's own registrations number in the dozens), inference that runs on the phone because there is no backend and there will not be one, and the model's weight inside an APK that ships an 854K catalogue today. Worth designing only against a named corpus and a size budget.
- **A screenshot or UI test layer.** There is none by choice, and the cost shows up as UI defects that only a device pass finds. The 2026-09-06 pass is the evidence either way: it caught three (a launcher icon that rendered as a photo on a white tile, a caught species drawing a blown-up silhouette, an uncaught neighbour masked as `? ? ?` beside its own binomial), and none of them would have failed an assertion anybody would have thought to write — they were all *wrong-looking*, not wrong. That is an argument for screenshots specifically rather than UI tests generally. Worth revisiting only if the phone loop stops being practical.
