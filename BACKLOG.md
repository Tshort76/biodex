...
- **A tree of life page.** Browse the catalogue by taxonomy rather than by grid position, with the class and kingdom vocabulary the grid's filters already use picking which branch to open. The data cost is near zero — GBIF's `species/match` response already carries kingdom, phylum, class, order and family for every species, and it is already in `tools/catalogue/cache/`, so no new request is needed. Mockups exist and the shape is designed but not chosen; the open question is whether the growth-form facets (`tree`, `shrub`, `herb`) belong on a page about clades at all, since they cut across five plant orders.
