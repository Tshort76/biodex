...
- **A relatives graph, and a lineage chain on the entry screen.** The want is to see how a species relates to the others in the dex, not to look up where it sits — a browsable tree of the whole catalogue was mocked up and turned down for answering the wrong question. The proposal is a cladogram centred on one species, reached from a Lineage section on that species' own entry page (and from a node-link icon in the grid's top bar). The data cost is near zero: GBIF's `species/match` response already carries kingdom, phylum, class, order and family for every species and is already in `tools/catalogue/cache/`, so no new request is needed. The design constraint worth knowing before building is that **101 of the 230 species are the only member of their family here**, so the graph's radius has to walk outward until it finds someone rather than being fixed, and branches that do not lead to the focal species fold into a single node carrying their count.
