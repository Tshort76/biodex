...
| **BioDex is a share target** — `ACTION_SEND` for `image/*` and `text/plain` opens Register above the grid with the photo attached and the name in the search box. The intent is parsed by a pure function (`shareIntakeFrom`) and carried as route arguments, so the screen has one way in. A shared photo is force-copied at registration, because a share grant cannot be persisted. | `ui/nav/ShareIntake.kt`, `MainActivity.kt`, `AndroidManifest.xml`, `ui/register/`, `data/photo/CaptureRegistrar.kt` | `M45`, `D39` |
