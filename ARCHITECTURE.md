...
| **Older sightings take their place from the photo (v22.5)** — `SightingPlaceSweep` (`data/photo/`) over `PlaceBackfillStore` (`CaptureDao.placelessPhotographed`, `fillPlace` with `COALESCE(locationLabel, :label)`), decision in pure `planPlaceBackfill`; wired in `AppContainer.startCatalogueImport` after `UserNameSweep`, gated on `hasPhotoLibraryAccess` and one-shot behind `meta.placeBackfillDone` (`MetaEntity.KEY_PLACE_BACKFILL_DONE`), set at the end of the first run. `CaptureRegistrar.relink` applies the same plan to the row it re-links. `CaptureStore` gains `applyPlaceBackfill`. | `data/photo/SightingPlaceSweep.kt`, `data/photo/CaptureRegistrar.kt`, `data/db/Daos.kt`, `data/repo/DexRepository.kt`, `AppContainer.kt` | `M13`, `D56`, `D67` |
