...
| **Adding a species is its own action (v24)** — `DexGridUiState.addableName` is the trimmed search when no species in the dex matches it (`addableNameFor`, the grid's own fold); the header ＋ then navigates to `ConfirmSpecies` with that name, and otherwise to `Register(initialQuery = …)`. Register's no-results row does the same through `onAddSpecies`; the add-your-own button, its `RegisterEvent`, `canAddOwn` and `PlacePrompt.ADD_OWN` are gone. `AddSpeciesDraft` carries a name only; `AddSpeciesRegistrar.create` takes no captures and returns `Created(speciesId, dexNumber)`, so `UserSpeciesStore.deleteUserSpecies` (the rollback for a failed capture) went with them. After accept the card becomes `ConfirmSpeciesUiState.Added` and asks "Have you caught it?": *Not yet* pops to the grid, *Yes* opens `EntryDetail` with `popUpTo(DexGrid)`. | `ui/grid/DexGridState.kt`, `ui/grid/DexGridScreen.kt`, `ui/register/*`, `ui/addspecies/*`, `data/repo/AddSpeciesRegistrar.kt`, `data/repo/UserSpeciesStore.kt`, `ui/nav/NavGraph.kt` | `M08`, `M20`, `D69` |
