...
| **A white rim on the launcher icon** — the adaptive icon's background layer is white and the foreground is inset 21dp, so the rim follows the launcher's own mask shape instead of a ring baked into the art at a radius we guessed. | `res/mipmap-anydpi-v26/ic_launcher.xml`, `res/drawable/ic_launcher_art_inset.xml` | `D35` |
