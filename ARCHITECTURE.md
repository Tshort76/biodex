...
**R3 — Photo Picker strips EXIF location.** Confirmed behavior: the Android 13+ system picker redacts GPS metadata from returned URIs by default, and `ACCESS_MEDIA_LOCATION` does not apply to picker-authority URIs. So M13's location field will usually be null on modern devices; the EXIF *datetime* survives, so `takenAt` is unaffected. Stance: accept it — M13 says location comes "from EXIF only, if present", and `locationLabel` (user-typed) is the designed fallback. An August 2026 mainline picker update adds an "Include location info?" prompt, so newer devices may start returning GPS with no code change. Rejected: switching to raw `ACTION_OPEN_DOCUMENT` everywhere just to keep GPS (worse picker UX for one nullable field). *Mitigated (D56, 2026-09-12):* the user can type the place at registration, and the geocoder names a coordinate pair when nothing was typed. *Checked on the phone (D57, 2026-09-13):* `ACCESS_MEDIA_LOCATION` + `setRequireOriginal` does not work here — the provider answers *Require Original is not supported for Picker URI* — and asking for the permission raises the full photo-library prompt, so both were withdrawn. The picker's own location switch (`locationMetadataAccessRequested`, flag `PICKER_LOCATION_METADATA_ENABLED`) is present but off on this Pixel; when it ships, GPS arrives without any permission. *Mitigated again (D58, 2026-09-13):* the Files picker (`ACTION_OPEN_DOCUMENT`) is a second door on the Register screen; its `com.android.providers.media.documents` URIs open as a plain stream with GPS intact once `ACCESS_MEDIA_LOCATION` is granted (declared-but-denied reads redacted; a pass that seemed to work after `pm revoke` was MediaProvider's cached calling identity, cleared by the next install). The permission is requested on the Files button only. R3's original rejection stands only for making it the *only* picker.
...
|---|---|---|
| *(revised, see §12.2 — the second kingdom now)* **Fungi as a third kingdom** — own classes (mushroom, bracket, other fungus), silhouettes, meter, stats block and dex-number range. Not a flavour of plant. | `domain/`, `ui/stats/`, the pipeline's `curated_fungi.json` | `D27` |
| *(removed, see §12.3)* **An in-app camera** — `ACTION_IMAGE_CAPTURE` into app cache via FileProvider, promoted to the gallery at registration. Needs **no `CAMERA` permission**: the system camera app holds it, and declaring a permission you do not hold is what throws. | `data/photo/`, `ui/register/` | `D26`, `R22` |
| *(removed, see §12.2)* **Plant identification through Pl@ntNet** — opt-in per photo, plants only. One downscaled, re-encoded copy of one photo leaves the device (so EXIF and GPS go with the re-encode); candidates are checked against the GBIF backbone before display; the app never picks one. | `data/identify/`, `ui/register/` | `M36`, `D19`–`D23` |
| *(removed, see §12.2)* **A plant keeps no photograph of its own** — its tile shows the catalogue's reference image, and the same holds for a plant the user adds. Animals and fungi still keep theirs. | invariant in `data/repo/AddSpeciesRegistrar.kt`; side effects in `ui/addspecies/ConfirmSpeciesViewModel.kt` | `M41`, `D25` |
...
| *(revised, see the v24 row)* **The user's own species count** — a caught user-added species enters its kingdom's, class's and ecosystems' fractions on both sides, so 120 curated animals plus two of the owner's read `2/122`. `Meter.userAdded` stays as a breakdown of the numerator; the trailing `+N` column beside each meter is gone. | `domain/DexProgressMath.kt`, `ui/common/Meters.kt`, `ui/stats/` | `M02`, `M15`, `D29` |
| **Reference photographs are fitted, not cropped** — the detail hero and the confirm card's mini-hero letterbox the pulled image instead of filling the frame with the middle of it; the hero stops drawing its silhouette once the photograph is up. The user's own thumbnails still crop. | `ui/detail/EntryDetailScreen.kt`, `ui/addspecies/ConfirmCardScreen.kt` | `D30` |
| **Register moves into the grid's top bar** — the floating action button is gone, and with it the 88dp of bottom padding the grid carried to scroll clear of it. | `ui/grid/DexGridScreen.kt` | `D31` |
| **Sort by dex number or name** — a fourth dropdown beside the filters, held in its own flow rather than in `DexGridFilters` so clearing the filters cannot reorder the grid. `combine` is typed only to five inputs, so the five original reads compose into one value and the sort joins that. | `ui/grid/DexGridState.kt`, `DexGridViewModel.kt` | `M01`, `D32` |
| **A world range map on every entry** — a 128x64 equirectangular grid: the region carries one land outline as a base64 bitmask (~1KB, Natural Earth, public domain), each species carries the cell indices it shades (GBIF occurrence density, thresholded to drop stray records). Drawn as rects on a `Canvas`, so land and shading share cell edges. Schema v3 via `MIGRATION_2_3` (four `ADD COLUMN`s); the pipeline decodes PNG tiles with stdlib zlib rather than adding Pillow. | `tools/catalogue/build_catalogue.py`, `domain/Models.kt` (`RangeGrid`), `ui/common/RangeMap.kt`, `data/db/Migrations.kt` | `M04`, `M05`, `D34` |
| **A white edge on the launcher icon** — the art keeps its authored scale and white is painted over everything outside a superellipse fitted to the launcher's own mask, sized against the 72dp the mask keeps rather than the 108dp canvas. The inset-over-a-white-background version this replaces rendered as a square photograph on a white tile. `tools/icon/build_icon.py` generates all five densities from one source and documents how the mask was measured. | `tools/icon/build_icon.py`, `res/mipmap-*/ic_launcher_art.png`, `res/mipmap-anydpi-v26/ic_launcher.xml` | `D35`, `D37` |
| **A caught species with no picture is marked** — the grid cell's tick goes filled and green when the cell is drawing a silhouette rather than a photograph, so a deleted or revoked gallery photo stops reading as a species never caught. The rule is a pure function taking the load result, kept beside the chrome rule that deliberately refuses it. | `ui/common/TileState.kt`, `ui/common/DexComponents.kt` | `M44`, `D38` |
| **Nearest** — every species keeps its GBIF path in five nullable columns, and one screen names the three catalogue species closest to it with the hops between them (2 same family … 12 across kingdoms). The measure is pure and lives in `domain/`, so the whole scale is pinned on the JVM. Schema v4 via `MIGRATION_3_4` (five nullable `ADD COLUMN`s, no default to keep in step); catalogue asset v5. Reached from a binary-tree glyph in the grid's top bar and a link on an entry. | `domain/TaxonDistance.kt`, `ui/nearest/`, `ui/common/HopBadge.kt`, `data/db/Migrations.kt`, `tools/catalogue/build_catalogue.py` | `M43`, `D36` |
| **The unlock reveal actually animates** — it never did: `animateFloatAsState(targetValue = 1f)` is already at 1f on first composition, so the shipped reveal was a still card. Rebuilt as four `Animatable`s on one sequenced timeline — resolve, ring burst, staggered naming lines, counter tick with its own haptic. | `ui/reveal/UnlockRevealOverlay.kt` | `D8`, `D33` |
| **The grid draws the reference picture** — every caught cell leads with the catalogue's Wikimedia picture and keeps the user's own thumbnail as the fallback for when it has not cached; only when both fail does it draw the silhouette with M44's tick. The order is a pure list (`tileImageSources`) pinned on the JVM, and the composable walks it, advancing one place per load failure. An uncaught cell still never draws a picture. `TileState.CAUGHT_REFERENCE_IMAGE` was renamed `CAUGHT_NO_OWN_PHOTO`, since the reference picture stopped being what set that state apart. | `ui/common/TileState.kt`, `ui/common/DexComponents.kt` | `M01`, `M11`, `D39` |
| **The unlock reveal is a celebration** — confetti thrown from the halo on a ballistic arc, a warm radial wash, a sweep-gradient ring turning around the picture, and an amber label, on top of D33's sequence. The confetti is one full-screen `Canvas` above the column with a fixed seed, so it takes no input and scatters the same way every time; its colours live in `Confetti.kt`, not the theme. | `ui/reveal/Confetti.kt`, `ui/reveal/UnlockRevealOverlay.kt` | `D8`, `D33`, `D40` |
| **The uncaught tile is a dimmed picture** — an uncaught cell draws the same reference picture as a caught one through a zero-saturation colour filter at under half alpha; the silhouette is the fallback for both. `tileDrawsDimmed` is a pure function of the tile state, pinned beside the chrome rule. `gridThumbnailUrl` rewrites a Commons URL to its 960px rendition (pure, pinned), and the cell retries its list once after every candidate fails. | `ui/common/TileState.kt`, `ui/common/DexComponents.kt` | `M01`, `D41` |
| **The grid opens in name order** — `DexSort.DEFAULT` is `NAME`; the ViewModel, the state defaults and the Sort dropdown's cleared label all read it, so flipping the default is one line. | `ui/grid/DexGridState.kt` | `M01`, `D32`, `D42` |
| **User-added names are formatted at the door** — `formatCommonName` / `formatScientificName` are pure functions called from `SpeciesFields.normalized()`, which `create`, `backfill` and the card's `previewFields` all pass through. The card's hand-edit fields bind to the raw typed value (`Card.typedCommonName` / `typedScientificName`) so the formatter never edits under the cursor. | `domain/NameFormat.kt`, `domain/UserSpecies.kt`, `ui/addspecies/ConfirmSpeciesState.kt` | `M19`, `M45`, `D43` |
| **Existing user-added names are respelled on start** — `nameCorrections` (pure, `domain/NameFormat.kt`) lists the user rows whose spelling differs from the formatter's; `UserNameSweep` reads `SpeciesDao.userSpeciesOnce` and writes `rename` for each, in one transaction, from `startCatalogueImport` after the import. Idempotent, no flag. | `domain/NameFormat.kt`, `data/repo/UserNameSweep.kt`, `AppContainer.kt` | `M45`, `D45` |
| **The grid cell is one picture with the name on it** — `SpeciesCell` is a single 104dp `Box`: the photograph fills it (`ContentScale.Crop`, `Alignment.TopCenter`), and the name sits at the foot over a vertical scrim. `tileLabelOnPicture` decides scrim-versus-surface from whether the cell actually drew a picture, the same fact `tileWearsLoudTick` takes. No dex number. | `ui/common/DexComponents.kt`, `ui/common/TileState.kt` | `M01`, `M26`, `D46` |
| **The grid's order is a saved preference** — `AppSettings.dexSort` is a `StateFlow<String?>` over one `SharedPreferences` key holding `DexSort.wireName`; `DexGridViewModel` maps it through `dexSortFromWireName` (unknown → `DexSort.DEFAULT`) instead of holding its own `MutableStateFlow`, and both `onSort` and the Settings chips write the key. | `data/settings/AppSettings.kt`, `ui/grid/DexGridState.kt`, `ui/grid/DexGridViewModel.kt`, `ui/settings/SettingsScreen.kt` | `M01`, `D47` |
| **Game animals carry the edible tag** — `foodSource: true` in `curated_animals.json` becomes `uses: ["edible"]` and nothing else — no note, and `usesAttribution` stays null because that field credits Duke's and renders as "Medicinal: …"; `EntryDetailState.uses` no longer short-circuits on `Kingdom.ANIMAL`, so a tagged animal's entry shows the tag and the standing disclaimer. The set is pinned against the shipped asset by `AnimalFoodSourceTest`. | `tools/catalogue/build_catalogue.py`, `tools/catalogue/curated_animals.json`, `ui/detail/EntryDetailState.kt` | `M24`, `D14`, `D48` |
| **Clearing the caches asks first** — the button sets `confirmClear`, and the Material dialog calls `onClearCaches`; the sentence is `clearCacheConfirmBody(sizes)` in `SettingsState.kt`, pure and tested. | `ui/settings/SettingsScreen.kt`, `ui/settings/SettingsState.kt` | `S01`, `D49` |
| **The build refuses a content change under an unchanged version** — after validation, `build_catalogue.py` compares everything but `catalogueVersion` against the committed asset and exits naming `region.json` when they differ under the same number. Without it the asset ships, installs, and is ignored by every phone that already imported that version. | `tools/catalogue/build_catalogue.py` | `D48` |
| **The uncaught entry hero is a dimmed picture** — `heroVisual` gains `HeroVisual.DimmedReference`; being caught no longer decides whether the picture is requested, only how it is drawn, and every uncaught fallback reports `NOT_CAUGHT` so the note stays silent. The hero draws it through `DIMMED_FILTER` / `DIMMED_ALPHA`, now `internal` in `DexComponents.kt` and shared with the tile so the two cannot drift. | `ui/detail/HeroState.kt`, `ui/detail/EntryDetailScreen.kt`, `ui/common/DexComponents.kt` | `M05`, `D41`, `D52` |
| **Search forgives a typo** — `domain/SearchMatch.kt` folds both sides (NFD, letters and digits only, lower case) and then runs Sellers' approximate-substring DP with a transposition term, budget 0/1/2 by folded query length (<5 / 5–8 / ≥9). `matchesQuery` in the grid state calls it, so the Register screen gets it for free. | `domain/SearchMatch.kt`, `ui/grid/DexGridState.kt` | `M14`, `D54` |
| *(removed, see D69 — the button is gone)* **The add-your-own button looks live** — `GhostCta` draws accent-on-accentSoft with an accent border when enabled and the old faint-on-hairline only when disabled. | `ui/register/RegisterScreen.kt` | `M08`, `D55` |
| *(revised, see D69 — the add-your-own path no longer carries a photo or a place)* **Sightings: time and place per capture, listed on the entry** — a `PlaceField` on the Register screen feeds `locationLabel` through `CaptureRegistrar.register` and, via `RegisterEvent.AddOwnSpecies.place` → `AddSpeciesDraft.place` → `AddSpeciesRegistrar.create`, through the add-your-own card. (v20.1: the `ACCESS_MEDIA_LOCATION` request and the `setRequireOriginal` read were withdrawn after the phone test, see D57.) `PlaceNamer` (platform `Geocoder`, `None` in tests) names a coordinate pair only when nothing was typed. `ui/detail/SightingsState.kt` turns captures into rows — date · time of day, then label → coordinates → "Place not recorded" — and `SightingsList` draws them under the photo strip. No schema change: the columns date from v1. | `ui/register/*`, `ui/addspecies/*`, `data/photo/CaptureRegistrar.kt`, `data/photo/PlaceNamer.kt`, `data/photo/AndroidPhotoGateway.kt`, `ui/detail/SightingsState.kt`, `ui/detail/EntryDetailScreen.kt`, `AndroidManifest.xml` | `M13`, `D56`, `R3` |
| **Plants removed (v21)** — the kingdom, its 80 species, the Duke's data and Pl@ntNet identification are gone; `MIGRATION_5_6` deletes every `plant` row (captures, entries, ecosystem links, species — children first, since foreign keys are off during a migration) before any query maps a row through the enum; `planImport` skips archived species spelled `plant` / `tree` / `shrub` / `herb` / `fern` on the raw wire strings; `GbifClient.classify` drops a Plantae match; `AppSettings` removes the four identification preferences by name; the three Duke's columns stay on `species` holding their defaults. The Uses filter now follows tagged species rather than the plant meter. Catalogue v8: 224 animals, 30 fungi. | `data/db/Migrations.kt`, `data/db/AppDatabase.kt`, `data/backup/ImportMerge.kt`, `data/net/GbifClient.kt`, `data/settings/AppSettings.kt`, `domain/Models.kt`, `ui/grid/DexGridState.kt`, `tools/catalogue/build_catalogue.py` | `D59`, `M22`–`M27`, `M35`, `R21` |
| **From Files keeps the place** — a 📁 target in `PhotoAttachRow` launches `ActivityResultContracts.OpenDocument()` (image/*); the result goes through the same `persistGrant` → `PickedPhoto` path as the gallery picker. `ACCESS_MEDIA_LOCATION` is declared again and requested from that tap alone (`RequestPermission` → open the picker whatever the answer). `readExif` is the plain stream for every URI and logs the authority beside `gps=`; a documents-provider URI reads `gps=true` once granted. | `AndroidManifest.xml`, `ui/register/RegisterScreen.kt`, `data/photo/AndroidPhotoGateway.kt` | `M13`, `D58`, `R3` |
| **No media permission before the gallery picker** (v20.1; the permission itself is back for the Files button in v20.2, see the row above) — the v20 `ACCESS_MEDIA_LOCATION` request and `openForExif` are gone; `readExif` opens the plain stream. The picker refuses `setRequireOriginal` for its URIs and the request raised the photo-library prompt. | `AndroidManifest.xml`, `ui/register/RegisterScreen.kt`, `data/photo/AndroidPhotoGateway.kt` | `D57`, `R3` |
| **An uncaught hero loads at its final weight** — `HeroVisual.LoadingReference` carries `dimmed`, set from `!caught`: `heroNote` returns null for it, and the placeholder `SilhouetteIcon` is drawn through `Modifier.alpha(DIMMED_ALPHA)`. Removes both halves of the reported load glitch — the dark-shape-to-pale-picture jump, and the page shifting by the height of a caption that only existed while the fetch ran. | `ui/detail/HeroState.kt`, `ui/detail/EntryDetailScreen.kt` | `M05`, `D52`, `D53` |
| **The animal list is 224 species** — 104 more common Pacific animals appended to `curated_animals.json` as `#121`–`#224` (birds 121–152, mammals 153–166, fish 167–180, reptiles 181–186, amphibians 187–191, insects 192–209, other invertebrates 210–224), 26 of them carrying `foodSource`. Every name was matched against the GBIF backbone before the build ran. `catalogueVersion` goes to 7, which is what makes the phone import it. | `tools/catalogue/curated_animals.json`, `tools/catalogue/region.json`, `app/src/main/assets/catalogue/pacific.json` | `D1`, `D48`, `D50` |
| **The build's size checks are derived, not literal** — `validate` takes `expected_counts` built from the curated input files and asserts each kingdom's count matches and its `dexNumber`s are exactly `1..N`. `FungiCatalogueTest` compares distinct numbers to the stored size, and `CatalogueImporterRoomTest` asserts the three kingdoms partition the asset instead of naming their totals. | `tools/catalogue/build_catalogue.py`, `tools/catalogue/test_build_catalogue.py`, `data/catalogue/FungiCatalogueTest.kt` | `D51` |
| **An added species counts in the total (v24)** — `DexProgressMath.compute` counts every species row, the user's own whether or not caught; ecosystem members likewise. `Meter.userAdded` stays the caught user species, since it breaks down the numerator. Reverses the D29-era filter that kept an uncaught user species out of every fraction. | `domain/DexProgressMath.kt` | `M15`, `D29`, `D70` |
| *(revised, see §12.4 — the grid's ＋ picks a photo now, and adding by name is the empty search's link)* | **Adding a species is its own action (v24)** — `DexGridUiState.addableName` is the trimmed search when no species in the dex matches it (`addableNameFor`, the grid's own fold); the header ＋ then navigates to `ConfirmSpecies` with that name, and otherwise to `Register(initialQuery = …)`. Register's no-results row does the same through `onAddSpecies`; the add-your-own button, its `RegisterEvent`, `canAddOwn` and `PlacePrompt.ADD_OWN` are gone. `AddSpeciesDraft` carries a name only; `AddSpeciesRegistrar.create` takes no captures and returns `Created(speciesId, dexNumber)`, so `UserSpeciesStore.deleteUserSpecies` (the rollback for a failed capture) went with them. `heldMatch` compares the card's fields with the dex (folded scientific name, then common name) and sets `Card.alreadyHeld`, which swaps the accept button for "Open …" (`Event.OpenExisting`), so a typo that GBIF corrects cannot duplicate a held species; `nearMissFor` runs the grid's forgiving `SearchMatch.matches` on the typed name and sets `Card.nearMiss`, a "Did you mean …?" link that does not block the add. After accept the card becomes `ConfirmSpeciesUiState.Added` and asks "Have you caught it?": *Not yet* pops to the grid, *Yes* opens `EntryDetail` with `popUpTo(DexGrid)`. | `ui/grid/DexGridState.kt`, `ui/grid/DexGridScreen.kt`, `ui/register/*`, `ui/addspecies/*`, `data/repo/AddSpeciesRegistrar.kt`, `data/repo/UserSpeciesStore.kt`, `ui/nav/NavGraph.kt` | `M08`, `M20`, `D69` |
| *(removed, see §12.4 — the photo row went with the Register screen; Lens lives on Identify)* | **The photo row carries Lens (v24)** — `PhotoAttachRow` ends in 📁 and, with a photo attached, 🔍, which starts `lensIntentFor` (the image share addressed to `LensShareEntryPointActivity` in `com.google.android.googlequicksearchbox`) and falls back to `lensChooserFor` on `ActivityNotFoundException`; no `<queries>` entry is needed because nothing is resolved first. The full-width Lens banner and the "📍 Place read from the photo" line are gone, and `RegisterUiState.placeFromPhoto` with them. | `ui/register/RegisterScreen.kt`, `ui/register/RegisterState.kt` | `S06`, `D64`, `D72` |
| *(revised, see §12.4 — the ranking moved to Identify's `rankedMatches`)* **Adding needs a found species; Register ranks by best match (v24)** — `ConfirmSpeciesUiState.Card.found` (a selected candidate with a scientific name) gates `canAccept` for a new species; a backfill is exempt. The card returns early after its warning when a new species is not found, keeping only the near-miss link. The VM's offline branch now yields `LookupOutcome.Failed("offline")` instead of writing a pending row, and `Added.detailsPending` is gone. `SearchMatch.rank` (EXACT, PREFIX, WORD, CONTAINS, NEAR) orders `registerResults`, dex number breaking ties. | `ui/addspecies/*`, `domain/SearchMatch.kt`, `ui/register/RegisterState.kt` | `M14`, `M20`, `D73`, `D74` |
| **Caught on the day the photo was taken (v24)** — `SYNC_CAUGHT_AT_SQL` sets `entries.caughtAt` to the species' earliest `captures.takenAt`; `EntryDao.syncCaughtAt` runs it inside `applyRegistration`, and inside `applyDeletion` when the entry survives, and `syncAllCaughtAt` runs at the end of `applyImport`. `planRegistration` seeds a new entry with `capture.takenAt`. `MIGRATION_6_7` (database v7, schema identical to v6) runs the SQL once over every entry. | `data/db/Daos.kt`, `data/db/Migrations.kt`, `data/db/AppDatabase.kt`, `data/repo/DexRepository.kt`, `data/photo/CapturePlans.kt` | `M09`, `D75` |
| **Capture! on the entry (v24)** — `rememberGalleryPicker` (the gallery door alone: grant on the way in, photo-library permission on first tap) opens from the entry's Capture! button and ＋ tile. `EntryDetailViewModel.onPhotoPicked` reads the EXIF and either calls `CaptureRegistrar.register` at once or holds the photo in `awaitingPlace`, which exposes `placePrompt` (the Register screen's `placeSearchState`) to the now-`internal` `PlacePromptDialog`; dismissal releases the grant when no capture uses the URI. `CaptureEvent.Captured(isFirst)` drives the reveal or the "+1 photo" chip. `EntryDetail(capture = true)` opens the picker on arrival, used by the add card's "Yes". The route loses `onRegister`, and the "Learn more" `LinkRow` becomes a 🔍 box beside Capture!. | `ui/register/GalleryPicker.kt`, `ui/detail/EntryDetailViewModel.kt`, `ui/detail/EntryDetailScreen.kt`, `ui/register/RegisterScreen.kt`, `ui/nav/NavGraph.kt` | `M04`, `D76` |
| **Home after a capture (v24)** — `EntryDetailRoute.onCaptured(note)` fires on a repeat `CaptureEvent.Captured`, or on dismissing the reveal a capture raised (`homeAfterReveal`). The NavGraph writes `scrollToSpeciesId` and `arrivalNote` to the `DexGrid` back-stack entry's `savedStateHandle` and pops to it; `DexGridScreen` hoists a `rememberLazyGridState`, calls `scrollToItem` on the species' index once it is in the list, shows the note for 1.8 s, and clears both keys through `onArrivalHandled`. | `ui/detail/EntryDetailScreen.kt`, `ui/grid/DexGridScreen.kt`, `ui/nav/NavGraph.kt` | `D77` |
| **Identify from a photo (v24)** — the grid's ＋ is `rememberGalleryPicker` (now `ui/capture/`) navigating to `Identify(photoUri, displayName)`. `identifyUiState` (pure) ranks the dex with `rankedMatches` (D74's rule; empty until something is typed), reuses the grid's `addableNameFor`, and ~~offers `clipboardOffer`~~ (D79: `nameFromClipboard`, the first non-blank line when it is short, has a letter and no `://`, fills the search on the first focus after `onLensOpened`, and on no other). `IdentifyRoute` reads the clipboard on `LocalWindowInfo.isWindowFocused`, since Android lets only the focused app read it and Lens hands nothing back. `IdentifyViewModel` reads the EXIF on open; a capture or an add runs through `withPlace`, which waits for that read and raises the shared `PlacePromptDialog` only when there are no coordinates. A capture calls `CaptureRegistrar.register`; an add sends `IdentifyEvent.Add`, and the NavGraph puts an `AddSpeciesDraft` carrying a `DraftPhoto(uri, place)`. `ConfirmSpeciesViewModel` then registers that photo after `create` and sends `AddedAndCaptured`, and the card's button reads "Add and capture". Back without a capture releases the grant unless a capture uses the URI. The place prompt's state and dialog, `PickedPhoto`, `openInLens` and `PrimaryCta` moved to `ui/capture/` and `ui/common/`. | `ui/identify/`, `ui/capture/`, `ui/addspecies/`, `ui/grid/DexGridScreen.kt`, `ui/nav/NavGraph.kt` | `M07`, `M08`, `S06`, `D78` |
| **The place prompt suggests from an offline list (v23)** — `tools/places/build_places.py` filters the GeoNames US dump to California, Oregon and Washington and a whitelist of feature codes, deduplicates on name-and-state and writes `assets/places/pacific.tsv` (44,620 rows, 1.7 MB, `name<TAB>state<TAB>tier<TAB>lat<TAB>lng`, coordinates to four decimals). `AssetPlaceGazetteer` parses it once per process, lazily — the first time the Register screen is opened, not at start. The ranking is pure: `suggestPlaces` / `isKnownPlace` / `foldPlace` in `domain/PlaceSuggest.kt`, folded with word boundaries kept (unlike `SearchMatch.fold`, which strips them) so a match at a word outranks one mid-word. The prompt's `PlaceSearchState` is a separate `combine` chained onto `registerUiState` and run on `Dispatchers.Default`, because it scans 45,000 names per keystroke. It carries a `label` rather than a boolean: the list is autocomplete, so the button lights on anything typed, but a query the list holds is written as the *list's* label — `canonicalPlace` returns the matching spelling, and the fold that makes the match forgiving (case, accents, spacing) is exactly why the raw text must not be stored. `canonicalPlace` returns a `PlaceAnswer` (label plus optional point); `CaptureRegistrar.register` takes `placeLat`/`placeLng` and writes them only when the EXIF has no pair, so photo GPS is never overwritten. The first tier of suggestions is `CaptureDao.observePlaceLabels` — the collection's own labels, which is the only source that can hold a trail or a back garden. No schema change: the DAO query is a read. | `tools/places/`, `data/place/PlaceGazetteer.kt`, `domain/PlaceSuggest.kt`, `ui/register/` | `M13`, `D68` |
| **A re-link fills the sighting's place (v22.5, sweep removed in v22.6)** — `planPlaceBackfill` (pure, `CapturePlans.kt`) and `CaptureStore.applyPlaceBackfill` (`CaptureDao.fillPlace`, `COALESCE(locationLabel, :label)`), applied by `CaptureRegistrar.relink` after the reference swap. The one-time startup sweep that used the same plan ran on the owner's phone and was then deleted; its `placeBackfillDone` row stays in `meta`, unread. | `data/photo/CapturePlans.kt`, `data/photo/CaptureRegistrar.kt`, `data/db/Daos.kt`, `data/repo/DexRepository.kt` | `M13`, `D56`, `D67` |
| **The ball's upper half is a rainbow (v22.4)** — `PokeballColours.upperRainbow` replaces `upper`; `drawHalf` takes a `Brush`, and the upper half is `Brush.sweepGradient` over `rainbowStops` (pure, pinned: the arc occupies 0.5–1.0 of the sweep, the hidden lower half wears the first colour so the 3 o'clock seam is invisible). `RAINBOW_ARC` in the overlay is the confetti palette reordered into a spectrum. | `ui/reveal/Pokeball.kt`, `ui/reveal/UnlockRevealOverlay.kt` | `D62`, `D66` |
| **The reveal ignores the system animation scale (v22.3)** — the overlay's `LaunchedEffect` timeline runs inside `withContext(FullSpeedMotion)`, a `MotionDurationScale` of 1f, so a phone with "Animator duration scale" off still plays it. Nothing else in the app overrides the scale. | `ui/reveal/UnlockRevealOverlay.kt` | `M09`, `D65` |
| *(revised, see D69 — `ADD_OWN`, `canAddOwn` and `onAddOwnTyped` are gone; the prompt serves Register only)* **The place is a prompt on the tap (v22.2)** — `RegisterUiState` loses `place`/`placeHint` and gains `placePrompt: PlacePrompt?` (`REGISTER` / `ADD_OWN`), `placeFromPhoto` and `needsPlacePrompt`; `canRegister`/`canAddOwn` need only species-or-name, photo and a finished EXIF read. `RegisterViewModel.onRegister` / `onAddOwnTyped` raise the prompt when `needsPlacePrompt`, `onPlaceEntered` resumes the right one with the label, `onPlacePromptDismissed` clears it, and a `PlaceMissing` from the door re-raises it. `PlacePromptDialog` is a Material `AlertDialog` with one `BasicTextField`; the old `PlaceField` is gone. | `ui/register/RegisterState.kt`, `ui/register/RegisterViewModel.kt`, `ui/register/RegisterScreen.kt` | `M13`, `D60`, `D64` |
| **The gallery photo's place comes from the original (v22.1)** — `mediaStoreIdFromPickerUri` (pure, `PickerUris.kt`) extracts the id from a local picker URI; `AndroidPhotoGateway.readExif` opens `MediaStore.setRequireOriginal(Images.Media.EXTERNAL_CONTENT_URI/<id>)` first when `hasPhotoLibraryAccess` and falls back to the picker stream, keeping whichever read carried coordinates. `READ_MEDIA_IMAGES` and `READ_MEDIA_VISUAL_USER_SELECTED` are declared; the Register route asks for them plus `ACCESS_MEDIA_LOCATION` on the first gallery tap and, if a picked photo came back placeless, from a tappable line under the place field that re-reads it on grant. | `data/photo/PickerUris.kt`, `data/photo/AndroidPhotoGateway.kt`, `ui/register/RegisterScreen.kt`, `AndroidManifest.xml` | `M13`, `D63`, `R3` |
| **A capture needs a place (v22)** — `CaptureRegistrar.register` returns `RegisterResult.PlaceMissing`, writes nothing and releases the attempt's grant when the label is blank and the EXIF has no coordinates; the photoless path requires the label outright. `AddSpeciesRegistrar` maps it to `CreateResult.PlaceMissing` and rolls the species back. On the screen, `RegisterViewModel.onPhotoPicked` reads the EXIF on IO and sets `PickedPhoto.hasLocation` (null while reading); `RegisterUiState.placeSatisfied` gates `canRegister` and `canAddOwn`, and `placeHint` (`REQUIRED` / `READING` / `FROM_PHOTO`) drives the field's placeholder and note. The test fakes default to EXIF *with* coordinates so the rest of the suite is unaffected. | `data/photo/CaptureRegistrar.kt`, `data/repo/AddSpeciesRegistrar.kt`, `ui/register/RegisterState.kt`, `ui/register/RegisterViewModel.kt`, `ui/register/RegisterScreen.kt`, `ui/addspecies/ConfirmSpeciesViewModel.kt` | `M13`, `D60` |
| **Unlink the photo, keep the sighting (v22)** — `planPhotoUnlink` (pure, `CapturePlans.kt`) lists the files and the grant on deletion's shared-reference rule; `CaptureRegistrar.unlinkPhoto` applies it through the new `CaptureStore.clearCaptureReference` (`UPDATE captures SET photoUri = NULL, thumbPath = NULL, localCopyPath = NULL`). No schema change — both columns have been nullable since `MIGRATION_1_2`. `EntryDao.observeEntryStatuses`' fallback subquery adds `thumbPath IS NOT NULL`, so a photoless favourite falls through to another photo. The viewer offers it under the favourite, draws a 📍 frame for a photoless capture and hides favourite/unlink for it; the detail screen's strip tiles and sighting rows all open the viewer now. | `data/photo/CapturePlans.kt`, `data/photo/CaptureRegistrar.kt`, `data/photo/CaptureStore.kt`, `data/db/Daos.kt`, `data/repo/DexRepository.kt`, `ui/photoviewer/*`, `ui/detail/EntryDetailScreen.kt` | `M07`, `M12`, `S07`, `D61` |
| **The ball on the reveal (v22)** — `PokeballShell` is a `Canvas` over three clocks (`close`, `wobble`, `open`) drawn on the halo layer above the 150dp disc; the overlay adds two `Animatable`s and sequences hold → close (haptic) → rock → resolve, with the burst and confetti launched at the resolve. `wobbleAngle` is the one function with arithmetic and is pinned on the JVM. `REVEAL_DURATION_MS` is 3 900. | `ui/reveal/Pokeball.kt`, `ui/reveal/UnlockRevealOverlay.kt` | `M09`, `D62` |
| **A caught species can lead with the user's photo** — `entries.preferOwnPhoto` (schema 5, `MIGRATION_4_5`, `NOT NULL DEFAULT 0`) flows through `EntryStatusRow` into `SpeciesSummary.preferOwnPhoto`; `tileImageSources` puts the thumbnail first when it is set, and `heroVisual` takes an `ownPhotoModel` that wins over the reference URL. The toggle under the hero writes through `EntryDao.setPreferOwnPhoto`. `BackupEntry.preferOwnPhoto` defaults to false so older archives import; a merge keeps the local choice. | `data/db/Entities.kt`, `data/db/Migrations.kt`, `ui/common/TileState.kt`, `ui/detail/HeroState.kt`, `ui/detail/EntryDetailScreen.kt`, `data/backup/ImportMerge.kt` | `M01`, `M46`, `D44` |

Nearest is the clearest case yet for keeping the measure pure and in `domain/`: both of the rules that make it correct — skip a rank *both* species lack, break ties on the app's own class before dex number — were found by running it over the real catalogue and reading the output, not by reasoning about it, and both are pinned by JVM tests that need no device.
...
### 12.2 Plants, removed 2026-09-13

**The whole plant kingdom is gone, deliberately, and the identification feature with it.** A future reader who finds `PlantNetIdentifier`, `DukeIndex`, `curated_plants.json`, `TaxClass.TREE` or `Kingdom.PLANT` in this repository's git history is looking at a product decision (DESIGN.md D59), not an abandoned attempt.

**Why.** The owner: "I simply don't care to track them and have other apps for plant identification." The plants were the second kingdom from v4 (D12) to v20, and most of what was built between v4 and v6 existed for them — the growth-form classes, the two-tag uses model with its Dr. Duke's join and poison rule, the photoless plant catch (M41), and Pl@ntNet identification, which was plant-only by construction (D19, D23). With the kingdom gone, all of it was dead weight.

**What went.** `Kingdom.PLANT`, `TaxClass.TREE/SHRUB/HERB/FERN`, `PLANT_DEX_NUMBER_BASE`, `DexProgress.plants` and `EcosystemProgress.plants`; `PlantUse.MEDICINAL` (and the enum is `SpeciesUse` now, with `EDIBLE` alone); the three Duke's fields on `SpeciesDetail`, `SpeciesFields`, `LookupFields`, `CatalogueSpecies` and `BackupSpecies`; `silhouetteResOverride` and the conifer/broadleaf split (`defaultSilhouetteFor` is `sil_<class>` for every class); `keepsOwnPhoto` and the M41 branches in `AddSpeciesRegistrar`, `RegisterState` and `RegisterViewModel`; the whole `data/identify/` package, `IdentificationState`, `CandidatePanel`, the Register screen's Identify button, `AppContainer`'s registry and resolver, the Settings "Identification" section, `SettingsUiState`'s key and count fields, and `AppSettings`' four preferences (removed by name in `init`, never read); `DukeIndex`, `duke_ethnobot.json`, `LICENSE-duke.txt` and the Licenses screen's Duke's section; `GbifClient.synonyms`, the Plantae vernacular pass, `defaultPlantClass` and `plantSilhouetteFor`; `SpeciesLookupRepository`'s Duke's parameter, `POISON_CAUTION` and `derivedUses`; the confirm card's `UsesEditor`, Duke's line and plant photo warning (its kingdom toggle is animal↔fungus now); the five `sil_*` plant drawables; in the pipeline, `build_plant`, the Duke's download/index/asset/validation, `gbif_synonyms`, `plant_silhouette`, the Poison→Caution rule and `plant_uses_review.txt`. Tests that only pinned the above went with them: `DukeIndexTest`, `RealDukeAsset`, `data/identify/*`, `PlantUiTest` and `TwoKingdomFixture`, `IdentifyStateTest`, `PhotolessRegisterTest`, `IdentificationSettingsTest`, and the plant halves of a dozen more.

**What stayed, and why.** The `UsesSection`, `UsesNote.cautionSplit`, `keptUsesNote` and the Food source filter — D48 had made them animal features, and the fungal cautions (M35) render through the same section. The filter was gated on `plants.total > 0`, which would have hidden it the day the plants left; `DexGridUiState.showUseChips` now follows the tagged species, with a test. The nullable `captures.photoUri` / `thumbPath` columns and every photoless-capture state (`PhotoRef.None`, the export's `neverHadPhoto`), because a photoless capture is a general shape now, not a plant one. The 2001–2080 dex-number block, empty. `OkHttp` — it was never identification-only.

**The data.** Three doors could have let a `plant` row survive into an app whose `Kingdom.fromWireName` answers `ANIMAL` for it, and all three are closed. (1) **`MIGRATION_5_6`** deletes captures, entries, species_ecosystems and species where `kingdom = 'plant'`, children first because Room turns foreign keys on only after migrating; the statements live in `PURGE_PLANTS_SQL` and were dry-run against a copy built from `5.json`'s `createSql` (only the plant trees went). The schema is unchanged — `6.json` is `5.json` at a new version — and the three Duke's columns stay on the table with their defaults, because dropping them is a table recreate against four dependents for nothing a default does not already give. The reconciler could not do this job: it keeps a caught species whatever the asset says (§3.3), never names a user-added one, and runs after the first reads. (2) **`planImport`** skips an archived species spelled `plant`, or classed `tree`/`shrub`/`herb`/`fern`, on the raw strings before any enum sees them, counts it as `plantSpeciesSkipped`, and the import summary says so. (3) **`GbifClient.classify`** returns null for Plantae, so neither the backbone match nor the vernacular search can hand the confirm card a plant; `Fungi` classifies as `FUNGUS` / `OTHER_FUNGUS`, which it never did before (it fell back to animal). The importer runs on catalogue v8 the same launch, and finds no plant rows to keep.

**Documents.** DESIGN.md is at v21: M22–M27, M30, M35, M42, S10 and S15 revised in place; M31–M34, M36–M39, M41, S13, C09, C10 and C12 struck; D12 and D19–D25 marked superseded; D1, D3, D13–D15 and D27 marked revised; D59 new. The §2 definitions and tables read animal-and-fungus. `README.md`, `tools/catalogue/README.md`, `licenses.md` and `CLAUDE.md` no longer describe plants, Duke's or Pl@ntNet. R3's picker-redaction analysis and D58's Files-picker door are unaffected. `DESIGN-identification.md` at the root is untracked and untouched.

### 12.3 The in-app camera, removed 2026-09-24

**Why.** The owner never used it ("I will never use the camera icon") and wanted its slot on the photo row for Google Lens (D72). Photos come from the gallery or the Files picker, which is also where the place survives (D63).

**What went.** The Register screen's 📷 and its `TakePicture` launcher; `data/photo/CameraCapture.kt` (`PhotoSourceKind`, `shouldPromoteToGallery`, `shouldDeleteCacheFile`, the cache-path helpers) and its test; `PhotoGateway.newCameraCaptureUri`, `promoteToGallery` and `sweepCameraCache` with their `AndroidPhotoGateway` bodies and fakes; the start-up sweep in `AppContainer`; `PickedPhoto.source`; `CaptureRegistrar.register`'s `exifUri`; and the `capture` entry in `res/xml/file_paths.xml`.

**What stayed.** The FileProvider itself, for backup exports (S01). Photos already promoted into `Pictures/BioDex/` are ordinary gallery photos and their captures are untouched. No schema change.

**Documents.** DESIGN.md v24: M40 struck, M07 revised, D26 superseded, D71 new.

### 12.4 The Register screen, removed 2026-09-24

**Why.** Once a capture started from a photo (D78) and from an entry (D76), and adding by name had its own door (D69), Register had no job the other screens did not already do — and two capture screens would drift. The owner asked for anything left unused to go.

**What went.** `ui/register/` whole: `RegisterScreen`, `RegisterState` (`registerUiState`, `registerResults`, `RegisterUiState`, `RegisterEvent`, `PlacePrompt`), `RegisterViewModel`, and `RegisterStateTest`. The `Register` route, `EntryDetail.photoAdded` and the `preselectedSpeciesId` / `initialQuery` arrivals. The 📁 Files door (`OpenDocument`) and its subtitle. The grid ＋'s two jobs. `PickedPhoto.hasLocation`, since each screen now reads the EXIF itself.

**What moved.** `PickedPhoto`, `PlaceSearchState`, `placeAnswerFor`, `placeSearchState` and the place dialog to `ui/capture/PlacePrompt.kt`; the gallery picker to `ui/capture/GalleryPicker.kt`; Lens to `ui/capture/Lens.kt`; `PrimaryCta` to `ui/common/`. The place-prompt tests moved to `PlacePromptTest`, the ranking test to `IdentifyStateTest`.

**What stayed.** `ACCESS_MEDIA_LOCATION` in the manifest, which D63's gallery read still needs. No schema change.

**Documents.** DESIGN.md v24: M07, M08, M14, M16, S06, S14 and D31 revised; M28 struck; D58 superseded; D78 new.
