...
class BackupService(
    ...
    suspend fun import(archiveUri: String): ImportResult {
        val manifest = readManifest(archiveUri)
            ?: return ImportResult.Failed("That file is not an Animal Dex archive.")
...
/** `animaldex-backup-2026-09-01-0941.zip` — sortable, and readable in a file manager. */
fun exportFileName(at: Long): String {
    val stamp = SimpleDateFormat("yyyy-MM-dd-HHmm", Locale.US).format(Date(at))
    return "animaldex-backup-$stamp.zip"
}
