...
- **M07** — The Register a Species screen searches the catalogue — both kingdoms — by common or scientific name substring, entirely offline, then lets the user attach one photo via the system photo picker and register. There is no in-app camera anywhere in the app.
...
- **M23** — The filter chip row carries kingdom chips (Animals, Plants) and use chips (Edible, Medicinal) alongside the caught, class and ecosystem chips. Class chips shown are those of the selected kingdom, or all classes when no kingdom is selected. A use chip matches a plant tagged with that use (a plant tagged both matches either); animals never match a use chip. All five dimensions compose with each other and with search.
- **M24** — When a plant has at least one use, its detail screen shows a uses section between the habitat text and the photo strip. It shows, in this order: the use tags; the curated note, with any `Caution:` sentence emphasised in the stop colour **above** the rest of the note; when the plant is medicinal, one line naming the source, the record count and the activities ("Duke's records 105 traditional uses: astringent, diuretic, wound…"); and the one-line foraging disclaimer of M30 with the Duke's attribution. The edible tag and the note are visibly the app's curated content; the medicinal line is visibly a source's, in the attribution register. A plant with no recorded use shows nothing in that slot: habitat is followed directly by the photo strip.
...
- **M30** — The app's only claim about a plant's uses is that the use is *documented for the species* — by the curator (edible, the note) or by Duke's (medicinal), and the screen says which. Every plant detail with a use tag shows a one-line disclaimer beneath the uses section ("Documented uses of the species — not identification, not safety advice. Never eat or use a plant on the strength of this app."), and the Settings About section repeats it. The app never states that a photographed individual is that species, never states a part is safe, and carries no "safe to eat" wording anywhere.
...
*Edible is curated.* Duke's is overwhelmingly medicinal — 15 `Food` and 14 `Fruit` records in the entire file — and no other public source is both structured and trustworthy. So the fruit-bearing and edible plants stay editorial judgment in the curated list, exactly as ecosystem tags are, with a short curated note naming the part and the season, because "edible" alone is useless in front of a plant whose leaves are eaten and whose berries are not. The pipeline pulls Wikipedia's "Uses" and "Culinary" sections into a curator review file to help write those notes, and never into the asset.
*Poison records drive the cautions, as a checklist, not a badge.* The same Duke's file carries 1,654 `Poison` records. A "toxic" tag on some species would imply the untagged ones are safe — the wrong inference for a foraging app — so there is no tag. Instead the pipeline treats a `Poison` record as a build-time requirement: every such species must carry a `Caution:` sentence in its note or the build fails, so the set of cautioned species is decided by a source rather than by whoever wrote the notes, and a plant with both a traditional use and a poison record — precisely the case a hand-written list misses — cannot ship without one. The curator still adds cautions Duke's cannot express (a lethal lookalike, a preparation hazard, a documented use the modern literature says not to follow). On a user-added plant, where no curator exists, the bundled index pre-fills the same caution sentence (M27).
The safety dimension, stated plainly: an app that says "edible" carries real-world risk. BioDex therefore claims only that a use is *documented for the species*, says by whom, does not identify the plant in the photo (D2), does not say a part is safe, and shows a one-line disclaimer under every uses section (M30). Uses are plant-only: whether an animal is "edible" is a hunting and fishing regulation question, not a field-guide fact, and the app stays out of it.
...
- In-app identification of any kind — no classifier, no suggestions (Google Lens is the external tool for this). For plants especially: the app never tells the user what a plant is.
...
- Further kingdoms — fungi, kelps and seaweeds (C07).
